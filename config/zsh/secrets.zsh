...
if [[ -z "$OP_SERVICE_ACCOUNT_TOKEN" ]]; then
  echo "⚠ secrets: op-service-account-token missing from keychain — ANTHROPIC_API_KEY not loaded"
  echo "  Fix: security add-generic-password -a \"\$USER\" -s op-service-account-token -w ops1_... -T /usr/bin/security"
else
