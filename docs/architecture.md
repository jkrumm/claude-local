...
| `warden` | **The control plane.** Ingests signals, decides, drives the lifecycle to a verified outcome, and holds the only ledger (`~/.warden/warden.db`). Extracted from `hermes-agent` 2026-09-09 — a control plane cannot live inside the thing it supervises. Four LaunchAgents, never gateway cron. `DESIGN.md` is authoritative, `STATE.md` is where the build actually is |
...
| `com.jkrumm.warden-loop` | 600s | The alert triage act-loop — turns deduplicated `~/.warden/warden.db` events into one card per problem in `#agents` and a sideclaw `investigate` episode in the owning repo. Lives in **`warden`**, extracted from `hermes-agent` 2026-09-09. **A LaunchAgent and not a gateway cron on purpose:** the loop that notices Hermes is broken cannot depend on Hermes being up, and its Slack delivery is a plain HTTP client, not the gateway's live connection. |
...
runs an in-process cron layer — 6 live jobs (morning briefing, evening report,
watchdog, dispatch sweep, brain drift audit, project narratives; the
agents-overview digest was paused 2026-09-08) registered with `hermes cron`, invisible to launchd and to
