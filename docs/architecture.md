...
| `com.jkrumm.hermes-triage` | 600s | The alert triage act-loop — turns deduplicated `watchdog.db` events into one card per problem in `#agents` and a sideclaw `investigate` episode in the owning repo. **A LaunchAgent and not a gateway cron on purpose:** the loop that notices Hermes is broken cannot depend on Hermes being up, and its Slack delivery is a plain HTTP client, not the gateway's live connection. |
