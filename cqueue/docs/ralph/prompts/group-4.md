...
1. Read all existing files: `cqueue/src/` and `cqueue/server/` (Glob)
...
- Card shows: repo name (bold), workspace label (SourceRoot / IuRoot), badge for queue count (if >0), badge "notes" (if cnotes.md exists)
...
```
feat(cqueue): add React skeleton — routing, theme, repo list, git status bar
```
