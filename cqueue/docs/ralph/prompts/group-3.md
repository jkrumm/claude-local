...
1. Read existing server files from Groups 1–2 (Glob `cqueue/server/**/*.ts`)
...
4. Read `cqueue/PRD.md` — Sync Strategy section for atomic write semantics
...
GET: read `<path>/cqueue.md`, parse with `parseQueue()`, return array.
...
The SSE endpoint watches both `cqueue.md` and `cnotes.md` for a given repo path:
...
```
feat(cqueue): add queue/notes API routes and SSE file-watch endpoint
```
