...
1. Read `cqueue/PRD.md` — API section and File System Integration
2. Read `cqueue/server/index.ts` from Group 1 — understand the current stub
...
- For each: check if `cqueue.md` or `cnotes.md` exists
...
For `/api/repo`: if `cqueue.md` doesn't exist, create an empty one. Same for `cnotes.md`. This mirrors `cq`'s behavior.
...
```
feat(cqueue): add queue parser, repo scanner, and git status server routes
```
