...
Scaffold the entire cqueue project skeleton: package.json with all dependencies, TypeScript config, Vite config, Dockerfile, docker-compose.yml, Makefile, .gitignore, and `.env.example`. No application logic yet — just the foundation that lets all future groups build without surprises.

---

## Research & Exploration First

1. Read `cqueue/PRD.md` — understand the full stack and Docker setup
...
shell:
	docker compose --env-file ../.env exec cqueue sh
```
...
docker build -t cqueue-test . && docker rmi cqueue-test   # image must build
```
...
```
feat(cqueue): scaffold project skeleton — Bun/Elysia/React/Vite/Docker
```
