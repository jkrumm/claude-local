...
1. Read all existing `cqueue/src/` files — understand the current state
...
In `RepoList.tsx`, add:
- Page title "cqueue" in Blueprint `Navbar` (Geist Sans, large)
...
```
feat(cqueue): add notes editor, SSE polish, theme toggle — complete MVP
```
