# cqueue — RALPH Shared Context

You are implementing: **cqueue**, a local web dashboard for Claude Code's per-repo AI workflow files (`cqueue.md` and `cnotes.md`).

Read this fully before starting your group. The PRD is the authoritative spec: `cqueue/PRD.md`.

---

## What cqueue Is

cqueue is a persistent local service (Docker, port 7705) that provides a purpose-built browser UI for managing the Claude Code task queue and session notes for any local repo. The queue (`cqueue.md`) is a plain markdown file consumed by the Claude Code Stop hook. The notes (`cnotes.md`) are freeform session notes alongside each project.

Key invariants:
- **No backend state** — every truth lives in the files; the server is stateless
- **CLI compatibility** — `cqueue.md` format must not change (tasks separated by `\n---\n`)
...
GET  /api/queue?path=...                 Parse cqueue.md → task array
PUT  /api/queue?path=...                 Write task array → cqueue.md
GET  /api/notes?path=...                 Read cnotes.md raw
PUT  /api/notes?path=...                 Write cnotes.md
...
Atomic write: write to `.cqueue.md.tmp`, then `fs.rename()`. Both `.tmp` files are in the global gitignore.
...
**Primary (run after every group from inside `cqueue/`):**
```bash
...
```bash
docker build -t cqueue-test . && docker rmi cqueue-test
```
...
1. Reading the PRD: `cqueue/PRD.md`
